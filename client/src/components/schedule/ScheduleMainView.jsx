...
// //           {/* Navigation Buttons - Only for Day View */}
// //           {isDayView && !isThreeMonthView && !isMonthView && (
...
//         <ThreeMonthView
//           allPastEntries={allPastEntries}
//           scheduleEntries={scheduleEntries} // ✅ Passed this to ensure instant UI updates
//           employees={employees}
//           shiftTypes={shiftTypes}
//           leaveTypes={leaveTypes}
//           loading={threeMonthLoading}
...
const ScheduleMainView = ({
  ...
  setEmployeeSearch,
  orderedEmployees,
  filteredEmployees, // We keep this prop but will use orderedEmployees for rendering to keep the order intact
  scheduleEntries,
  shiftTypes,
  openEditModal,
  userRole,
  ...
  // ✅ Auto-scroll to the searched employee
  useEffect(() => {
    if (employeeSearch && employeeSearch.trim() !== '' && !isMonthView && !isThreeMonthView) {
      const searchLower = employeeSearch.toLowerCase();
      const firstMatch = orderedEmployees.find(emp =>
  ...
  }, [employeeSearch, orderedEmployees, isMonthView, isThreeMonthView]);
  ...
  const downloadScheduleData = async () => {
    ...
    // Export the full ordered list, not just the filtered list
    const displayedEmployees = orderedEmployees;
    const displayedDates = weekDays;
  ...
  const handleDragEnd = (event) => {
    const { active, over } = event;
    // ✅ Safety check and strictly using orderedEmployees
    if (active && over && active.id !== over.id) {
      const oldIndex = orderedEmployees.findIndex(emp => emp.id === active.id);
      const newIndex = orderedEmployees.findIndex(emp => emp.id === over.id);
      if (oldIndex !== -1 && newIndex !== -1) {
        handleReorder(oldIndex, newIndex);
      }
    }
  ...
  const handleMouseMove = (e) => {
    ...
    if (draggedOverCells.length > 0) {
      ...
      const allEmployeeIds = orderedEmployees.map(emp => emp.id); // ✅ Use orderedEmployees
      const startEmpIndex = allEmployeeIds.indexOf(parseInt(startEmpId));
      const endEmpIndex = allEmployeeIds.indexOf(parseInt(endEmpId));
  ...
  }, [isDragging, dragStartCell, selectedTemplate, weekDays, orderedEmployees]);
    ...
    <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl border border-slate-200 p-3 sm:p-4 lg:p-6">
      ...
      {!isMonthView && !isThreeMonthView && (
        <div className="mb-4 sm:mb-6">
          <input
            type="text"
            placeholder="Search employees to highlight..."
            value={employeeSearch}
            onChange={(e) => setEmployeeSearch(e.target.value)}
            className="w-full p-2 sm:p-3 border border-slate-300 rounded-xl text-sm sm:text-base text-slate-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white"
          />
  ...
  >
    ...
    >
      ...
      >
        ...
        <tbody ref={tableBodyRef}>
          {/* ✅ Using strictly orderedEmployees to keep the order fully intact */}
          <SortableContext
            items={orderedEmployees.map(emp => emp.id)}
            strategy={verticalListSortingStrategy}
          >
            {orderedEmployees.map((emp, idx) => (
              <SortableEmployeeRow
                ...
                employeeSearch={employeeSearch} // ✅ Passed search string for highlighting
                userRole={userRole}
                currentSchedule={currentSchedule}
                moveEmployee={moveEmployee}
                duplicateShiftForWeek={duplicateShiftForWeek}
