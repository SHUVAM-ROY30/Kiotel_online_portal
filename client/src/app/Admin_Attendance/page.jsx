...
//             inRowData.push(rec.clock_in || '');
//             outRowData.push(rec.clock_out === 'Missed' ? 'Missed' : (rec.clock_out || ''));
...
//               cell.font = { size: 9, color: { argb: cell.value ? COLORS.green : COLORS.gray200 } };
//             } else if (subIdx === 1) {
//               // Highlight Missed differently or same as missed clock out
//               cell.font = { size: 9, color: { argb: cell.value === 'Missed' ? COLORS.orange : (cell.value ? COLORS.red : COLORS.gray200) } };
//             } else if (subIdx === 2) {
//               const val = String(cell.value || '');
//               if (val.includes('L:') && val.includes('E:')) {
//                 cell.font = { size: 8, bold: true, color: { argb: COLORS.purple } };
...
//     earlyClockOut: dailyData.filter(e => e.status === 'Early Clock Out' || e.status === 'Late & Early').length,
//     absent: dailyData.filter(e => e.status === 'Absent').length,
//     missed: dailyData.filter(e => e.status === 'Missed').length
//   };
...
//                               </td>
//                               <td className="px-2 sm:px-3 py-2">
//                                 <span className={`inline-flex px-1.5 py-0.5 rounded ${row.clock_in ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'} text-[10px] sm:text-xs font-medium`}>
//                                   {formatTime(row.clock_in)}
//                                 </span>
//                               </td>
//                               <td className="px-2 sm:px-3 py-2">
//                                 <span className={`inline-flex px-1.5 py-0.5 rounded ${row.clock_out ? 'bg-green-50 text-green-700' : (row.is_missed ? 'bg-red-50 text-red-700' : (row.clock_in ? 'bg-gray-100 text-gray-500' : 'bg-gray-100 text-gray-500'))} text-[10px] sm:text-xs font-medium`}>
//                                   {row.is_missed ? 'Missed' : (row.clock_out ? formatTime(row.clock_out) : '—')}
//                                 </span>
//                               </td>
//                               <td className="px-2 sm:px-3 py-2"><StatusBadge status={row.status} /></td>
//                               <td className="px-2 sm:px-3 py-2 text-right text-xs sm:text-sm font-semibold text-amber-700">{row.late_minutes || '—'}</td>
...
//     earlyClockOut: uiDailyData.filter(e => e.status === 'Early Clock Out' || e.status === 'Late & Early').length,
//     absent: uiDailyData.filter(e => e.status === 'Absent').length,
//     missed: uiDailyData.filter(e => e.status === 'Missed').length
//   };
