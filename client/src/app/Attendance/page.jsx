...
// //   // Wakes up the Speech API early so there's no delay when it actually needs to speak
...
// //   useEffect(() => {
// //     // Load voices early to prevent any initialization lag
// //     if (typeof window !== "undefined" && "speechSynthesis" in window) {
...
//       const clockIn  = dataStatus.data.clock_in;
//       const clockOut = dataStatus.data.clock_out;
//       const attendanceDate = dataStatus.data.attendance_date;

//       // ═══════════════════════════════════════════════════════════
//       // FRONTEND TIME RESTRICTION LOGIC
//       // ═══════════════════════════════════════════════════════════
//       if (attendanceDate && selectedShift.shift_name !== 'ADMIN') {
//         // Construct exact start and end times in IST using absolute dates
//         const exactShiftStart = new Date(`${attendanceDate}T${selectedShift.start_time}+05:30`);
//         const exactShiftEnd = new Date(`${attendanceDate}T${selectedShift.end_time}+05:30`);
        
//         // If it crosses midnight, the end time is technically the next day
//         if (selectedShift.is_cross_midnight) {
//           exactShiftEnd.setTime(exactShiftEnd.getTime() + 24 * 60 * 60 * 1000);
//         }

//         const nowMs = Date.now(); // Current device time in milliseconds
...
//             setMessage(`🚫 Cannot clock in yet. Check-in opens 2 hours before your shift starts (${formatTime(selectedShift.start_time)}).`);
//             setLoading(false);
//             return; // Blocks moving to action step
//           }
//           if (nowMs > exactShiftStart.getTime() + twoHoursMs) {
//             setMessage(`🚫 Clock-in closed. You are more than 2 hours late for your shift (${formatTime(selectedShift.start_time)}).`);
//             setLoading(false);
//             return; // Blocks moving to action step
//           }
//         } else if (status === "clocked_in") {
//           // CLOCK OUT RULES
//           if (nowMs > exactShiftEnd.getTime() + sixHoursMs) {
//             setMessage(`🚫 Cannot clock out. It has been more than 6 hours since your shift ended (${formatTime(selectedShift.end_time)}).`);
//             setLoading(false);
//             return; // Blocks moving to action step
//           }
//         }
...
//   // ... [Rest of your UI render code remains perfectly untouched] ...
//   if (!authChecked) {
//     return (
...
//       const clockIn  = dataStatus.data.clock_in;
//       const clockOut = dataStatus.data.clock_out;
//       const attendanceDate = dataStatus.data.attendance_date; // Clean string from backend
...
//   // NEW STATE: Temporarily holds the scanned user until they click "Yes, that's me"
...
//       // Save data temporarily and show the confirmation screen
