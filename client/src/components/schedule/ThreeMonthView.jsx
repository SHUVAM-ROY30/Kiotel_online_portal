...
//   // ─── 🚀 MASSIVE PERFORMANCE FIX 1: Group entries by employee instantly ───
//   const entriesByEmployee = useMemo(() => {
//     const map = {};
//     entriesToUse.forEach((entry) => {
//       if (!map[entry.user_id]) map[entry.user_id] = [];
...
//   // ─── 🚀 MASSIVE PERFORMANCE FIX 2: Pre-calculate daily shift counts in O(N) time ───
//   const dailyShiftCounts = useMemo(() => {
//     const counts = {};
    
//     // Initialize the dictionary for only the days currently visible on screen
//     days.forEach(d => {
//       counts[format(d, 'yyyy-MM-dd')] = { 1: new Set(), 2: new Set(), 3: new Set() };
//     });

//     // Loop through the 8,000 entries EXACTLY ONCE
//     entriesToUse.forEach(e => {
//       if (e.assignment_status !== 'ASSIGNED') return;
//       if (!counts[e.entry_date]) return; // Skip dates that aren't in this month!
...
//   // ─── Instant O(1) Lookup instead of 744,000 loops ───
//   const getEmployeeCountForShift = (day, shiftNumber) => {
//     const dateStr = format(day, 'yyyy-MM-dd');
...
// // Added ROLE_MAP to identify Agent Trainees
// const ROLE_MAP = {
//   1: "Admin", 2: "Agent", 3: "Manager", 4: "Client",
//   5: "HR", 6: "Office Admin", 7: "Agent Trainee",
// };

// // NOTICE: Added employeeRoles to the props!
// const ThreeMonthView = ({ allPastEntries, employees = [], shiftTypes, leaveTypes, loading, employeeRoles = {} }) => {
//   const [currentMonth, setCurrentMonth] = useState(startOfMonth(new Date()));
//   const [fetchedEmployeeOrder, setFetchedEmployeeOrder] = useState(null);
//   const [orderFetchError, setOrderFetchError] = useState(null);
...
//                   <tr key={emp.id} className="border-b border-slate-200 hover:bg-slate-50 transition">
//                     <td 
//                       // ✅ Applied darker yellow color here
//                       className={`p-3 font-medium border-r border-slate-200 sticky left-0 z-10 ${isAgentTrainee ? 'bg-yellow-100' : 'bg-white'}`}
//                     >
//                       {emp.first_name} {emp.last_name}
//                     </td>
...
//                         <td 
//                           key={dateStr} 
//                           // ✅ Apply yellow background to the whole row for Agent Trainees
//                           className={`p-1 border-r border-slate-200 text-center align-middle ${isAgentTrainee ? 'bg-yellow-100' : ''}`}
//                         >
//                           <div className="w-10 h-10 flex items-center justify-center mx-auto">
//                             <MonthViewCell date={day} myPastEntries={empEntries} shiftTypes={shiftTypes} uniqueId={emp.id} isReadOnly={true} />
//                           </div>
