...
// const API2 = process.env.NEXT_PUBLIC_API_BASE_URL; // for /api/user-email (same as user_fetch.jsx)
...
//   const isOvernightShift = shiftEndHour < shiftStartHour || (shiftEndHour === shiftStartHour && shiftEndMin < shiftStartMin);
...
//   // ===== Role (copied behavior from user_fetch.jsx) =====
//   const [userRole, setUserRole] = useState(null);
//   const [roleLoading, setRoleLoading] = useState(true);

//   // Decide permissions here:
//   // If role_id 8 is non-admin, block admin actions.
//   const canManage = userRole === 1; // change to [1,5].includes(userRole) if HR should manage too
...
//     if (!canManage) return; // 🔒 role restriction
...
//       // (omitted here for brevity? You asked "whole code", so leaving as-is would be huge;
//       // but since you pasted truncated monthly export in your message, I can’t restore the missing tail safely.)
...
//               Read-only access: export/edit/delete/waive-late are disabled for your role (role_id {String(userRole)}).
//             </div>
//           ) : null}
//         </div>
...
//               >
//                 {activeTab === 'daily' && (
//                   <div className="absolute bottom-0 left-0 right-0 h-0.5 sm:h-1 bg-gradient-to-r from-blue-600 to-blue-500"></div>
//                 )}
//                 Daily Attendance
//               </button>
//               <button
...
//                             <th className="px-2 sm:px-3 py-2 text-left text-[10px] sm:text-xs font-semibold text-white uppercase">
//                               Employee
//                             </th>
//                             <th className="px-2 sm:px-3 py-2 text-left text-[10px] sm:text-xs font-semibold text-white uppercase">
//                               Shift
...
//                               Status
//                             </th>
//                             <th className="px-2 sm:px-3 py-2 text-right text-[10px] sm:text-xs font-semibold text-white uppercase">
//                               Late
//                             </th>
//                             <th className="px-2 sm:px-3 py-2 text-right text-[10px] sm:text-xs font-semibold text-white uppercase">
//                               Early
...
//                               OT
//                             </th>
//                             <th className="px-2 sm:px-3 py-2 text-center text-[10px] sm:text-xs font-semibold text-white uppercase">
//                               Photo
//                             </th>
//                             <th className="px-2 sm:px-3 py-2 text-center text-[10px] sm:text-xs font-semibold text-white uppercase">
//                               Actions
...
//                                     <span
//                                       className="text-gray-300 font-light mr-1.5 -translate-y-0.5"
...
//                                       row.is_missed ? 'bg-red-50 text-red-700' : 'bg-gray-100 text-gray-500'
...
//                                 <div className="text-[10px] sm:text-xs text-gray-500">{row.unique_id}</div>
//                               </td>
//                               <td className="px-2 sm:px-3 py-2 text-right text-xs sm:text-sm font-semibold text-gray-900">
//                                 {row.total_working_days || '—'}
//                               </td>
//                               <td className="px-2 sm:px-3 py-2 text-right text-xs sm:text-sm font-semibold text-emerald-700">
//                                 {row.present}
//                               </td>
//                               <td className="px-2 sm:px-3 py-2 text-right text-xs sm:text-sm font-semibold text-amber-700">
//                                 {row.late}
//                               </td>
//                               <td className="px-2 sm:px-3 py-2 text-right text-xs sm:text-sm font-semibold text-orange-700">
//                                 {row.early_clock_out || 0}
//                               </td>
//                               <td className="px-2 sm:px-3 py-2 text-right text-xs sm:text-sm font-semibold text-purple-700 bg-purple-50/50">
//                                 {row.penalty_minutes || 0}
//                               </td>
//                               <td className="px-2 sm:px-3 py-2 text-right text-xs sm:text-sm font-semibold text-rose-700">
//                                 {row.absent}
//                               </td>
//                               <td className="px-2 sm:px-3 py-2 text-right text-xs sm:text-sm font-semibold text-blue-700">
//                                 {row.total_ot_minutes}
//                               </td>
//                               <td className="px-2 sm:px-3 py-2 text-right text-xs sm:text-sm font-semibold text-blue-700">
...
//       {/* Modals (restricted by open handlers, but keep render logic) */}
...
export default function AdminDashboard() {
  ...
  // Monthly export report selector (3 options total)
  const [monthlyExportType, setMonthlyExportType] = useState('monthly_detailed');
  // monthly_detailed = current report (unchanged)
  // missed_clockin = new report #1
  // penalty_over_100 = new report #2
  ...
  const dailySummary = {
    present: uniqueEmployeesForSummary.filter((e) => e.status === 'Present').length,
    late: uniqueEmployeesForSummary.filter((e) => e.status === 'Late' || e.status === 'Late & Early').length,
    earlyClockOut: uniqueEmployeesForSummary.filter(
      (e) => e.status === 'Early Clock Out' || e.status === 'Late & Early'
    ).length,
    absent: uniqueEmployeesForSummary.filter((e) => e.status === 'Absent').length,
    missed: uniqueEmployeesForSummary.filter((e) => e.status === 'Missed').length
  };
...
const exportMonthlyMissedClockInReport = async () => {
  ...
  try {
    ...
    if (!result.success || !result.data) {
      alert(result.message || 'Failed to fetch missed/not-clocked-in report.');
      return;
    }
    ...
    // helper: join all dates, wrap nicely
    const joinDatesWrapped = (dates) => {
      // Make it readable but compact: 8 dates per line
      const perLine = 8;
      const chunks = [];
    ...
    const summaryHeaders = [
      ...
      'NO_RECORD Count',
      'NO_RECORD Dates',
      'MISSED_CLOCKOUT Count',
      'MISSED_CLOCKOUT Dates',
      'Notes'
    ];
    ...
    employees.forEach((emp) => {
      ...
      const noRecordDates = incidents
        .filter((x) => x.reason === 'NO_RECORD')
        .map((x) => x.date)
        .filter(Boolean);

      const missedClockOutDates = incidents
        .filter((x) => x.reason === 'MISSED_CLOCKOUT')
        .map((x) => x.date)
        .filter(Boolean);
      ...
      const row = wsSummary.addRow([
        ...
        emp.name,
        totalIncidents,
        noRecordDates.length,
        noRecordDatesText,
        missedClockOutDates.length,
        missedDatesText,
        ''
      ]);
      ...
      // Adjust row height based on the bigger of the two lists (rough heuristic)
      const linesNo = String(noRecordDatesText).split('\n').length;
      const linesMissed = String(missedDatesText).split('\n').length;
      const lines = Math.max(linesNo, linesMissed, 1);
      wsSummary.getRow(row.number).height = Math.min(200, 18 + lines * 14);
    });
    ...
    wsSummary.columns = [
      ...
      { width: 26 }, // Name
      { width: 14 }, // Total
      { width: 16 }, // NO_RECORD count
      { width: 45 }, // NO_RECORD dates
      { width: 20 }, // MISSED count
      { width: 45 }, // MISSED dates
      { width: 18 }  // Notes
    ];

    wsSummary.autoFilter = {
      from: { row: summaryHeaderRow.number, column: 1 },
      to: { row: summaryHeaderRow.number, column: summaryHeaders.length }
    };
    ...
    employees.forEach((emp) => {
      ...
      list.forEach((d) => {
        ...
        if (d.reason === 'MISSED_CLOCKOUT') {
          reasonCell.font = { bold: true, color: { argb: COLORS.red }, size: 10 };
          reasonCell.fill = solidFill(COLORS.redLight);
        } else if (d.reason === 'NO_RECORD') {
          reasonCell.font = { bold: true, color: { argb: COLORS.orange }, size: 10 };
          reasonCell.fill = solidFill(COLORS.orangeLight);
        }
      });
    });
    ...
    saveAs(
      new Blob([buffer]),
      `Attendance_Monthly_MissedClockIn_${monthlyYear}-${String(monthlyMonth).padStart(2, '0')}.xlsx`
    );
  } catch (err) {
...
const exportMonthlyPenaltyOverReport = async (threshold = 100) => {
  ...
  const handleExport = async (type) => {
    ...
    if (type === 'monthly') {
      if (monthlyExportType === 'monthly_detailed') {
        await exportMonthlyStyled(); // unchanged
      } else if (monthlyExportType === 'missed_clockin') {
        await exportMonthlyMissedClockInReport();
      } else if (monthlyExportType === 'penalty_over_100') {
        await exportMonthlyPenaltyOverReport(100);
      } else {
                ...
                <div className="flex-shrink-0 px-3 sm:px-4 py-2.5 sm:py-3 border-b border-gray-200">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                      ...
                      {/* NEW: choose which monthly report to export (3 options) */}
                      {canManage && (
                        <select
