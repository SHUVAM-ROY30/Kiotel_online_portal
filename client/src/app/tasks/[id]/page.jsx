...
//     setLoading(true);
//     try {
//       // 1. Fetch Internal API Data First
...
//       // Helper to safely extract YYYY-MM-DD for comparison
//       const getDateStr = (d) => {
//         if (!d) return "";
...
//       // 2. Fetch External API Data if hk_task_id exists
...
//           // -- MERGE LOGIC FOR TASK --
//           // Internal API takes priority. Only fill from external if internal is empty/null.
...
//             if (isInternalEmpty) {
//               intTask[key] = extTask[key];
//             }
//           });

//           // -- MERGE LOGIC FOR OCCURRENCES --
//           intOccurrences = intOccurrences.map(intO => {
//             // Match internal and external occurrences by Date
...
//                 const isInternalOccEmpty = mergedO[key] === null || mergedO[key] === undefined || mergedO[key] === "" || (Array.isArray(mergedO[key]) && mergedO[key].length === 0);
//                 if (isInternalOccEmpty) {
//                   mergedO[key] = matchedExtO[key];
//                 }
//               });
//               return mergedO;
//             }
...
//           // Add any occurrences that exist in External API but are missing in Internal API
//           const internalDates = new Set(intOccurrences.map(o => getDateStr(o.occurrence_date)));
//           extOcc.forEach(eo => {
//             const extDateStr = getDateStr(eo.occurrence_date);
//             if (!internalDates.has(extDateStr)) {
//               intOccurrences.push(eo);
//             }
//           });
...
//         } catch (extErr) {
//           console.warn("External API merge failed. Continuing with internal data.", extErr.message);
//         }
//       }
...
//     setSelectedOcc(occ);
//     // If it's an external occurrence that doesn't exist internally yet, it might not have an occurrence_id
...
//     if (!selectedOcc.occurrence_id) {
//       alert("Cannot claim an external-only occurrence. It must be synchronized first.");
//       return;
//     }
...
//                     <tr key={inst.occurrence_id || inst.occurrence_uuid || Math.random()} className="hover:bg-gray-50 transition cursor-pointer group" onClick={() => openOccurrence(inst)}>
...
//             {/* Display merged external Agent Response if it exists */}
//             {selectedOcc.agent_response && (
//               <div className="px-5 py-3 bg-blue-50 border-b border-blue-100">
//                 <span className="text-xs font-bold text-blue-800 uppercase tracking-wider block mb-1">Agent Response:</span>
//                 <p className="text-sm text-gray-800">{selectedOcc.agent_response}</p>
//               </div>
//             )}
...
//             {/* Display merged external Images if they exist */}
//             {selectedOcc.images && selectedOcc.images.length > 0 && (
//               <div className="p-4 bg-white border-b shadow-sm z-10">
//                 <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Attached Images</h4>
//                 <div className="flex gap-3 overflow-x-auto pb-2">
//                   {selectedOcc.images.map((img, idx) => (
//                     <a key={idx} href={img.resource_url} target="_blank" rel="noopener noreferrer" className="flex-shrink-0">
//                       <img src={img.resource_url} alt={img.original_name || "Attachment"} className="h-20 w-20 object-cover rounded-lg border border-gray-200 hover:opacity-80 transition" />
//                     </a>
//                   ))}
//                 </div>
//               </div>
