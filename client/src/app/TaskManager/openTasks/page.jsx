...
import { FaGripLinesVertical, FaTasks, FaFilter, FaList, FaThLarge, FaCog, FaTag, FaUser, FaExclamationCircle, FaSearch, FaTimes,FaPlus } from "react-icons/fa";
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center space-x-3 flex-shrink-0">
                      ...
                      <Link
                        ...
                        className="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white text-sm font-medium rounded-lg shadow hover:shadow-md transition-all duration-300 transform hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                      >
...
//     <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 sm:p-6">
//       {/* Header */}
//       <header className="bg-white shadow-lg rounded-xl mb-6 p-4 sm:p-6">
//         <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
//           <div>
//             <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 flex items-center">
//               <FaTasks className="mr-3 text-blue-600" />
...
//             />
//           </div>
//           {/* View Toggle (Optional: Keep or remove if logo is present on the right) */}
//           {/* <div className="flex items-center gap-4">
//             <div className="flex bg-gray-100 rounded-lg p-1">
//               <button
//                 onClick={() => setViewMode("table")}
...
//               value={selectedState}
//               onChange={(e) => setSelectedState(e.target.value)}
//               className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200 bg-white hover:border-gray-400"
//             >
//               <option value="">All Statuses</option>
//               {taskStates.map((state) => (
//                 <option key={state.Id} value={state.Id}>
...
//       {viewMode === "table" ? (
//         // Table View with Resizable Columns
//         <div className="bg-white shadow-lg rounded-xl overflow-hidden" ref={tableRef}>
//           <div className="overflow-x-auto">
//             <table className="min-w-full divide-y divide-gray-200">
...
//                   <th 
//                     scope="col" 
//                     className="relative group px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider"
//                     style={{ width: columnWidths.tags }}
//                   >
//                     <div className="flex items-center justify-between">
//                       Tags
//                       <div 
//                         className="absolute right-0 top-0 bottom-0 w-2 cursor-col-resize hover:bg-blue-500 hover:bg-opacity-30 transition-colors duration-200 flex items-center justify-center"
//                         onMouseDown={(e) => startResizing('tags', e)}
//                       >
//                         <FaGripLinesVertical className="h-3 w-3 text-gray-500" />
//                       </div>
//                     </div>
//                   </th>
//                   {/* Task Title Column */}
...
//                           </div>
//                           {ticket.due_date && (
//                             <div className={`text-xs ${new Date(ticket.due_date) < new Date() ? 'text-red-600' : 'text-gray-500'}`}>
//                               {new Date(ticket.due_date) < new Date() ? 'Overdue' : 'Upcoming'}
//                             </div>
//                           )}
//                         </td>
...
// import { useRouter } from "next/navigation";
// import Link from "next/link";
// import { FaGripLinesVertical, FaTasks, FaFilter, FaList, FaThLarge, FaCog, FaTag, FaUser, FaExclamationCircle, FaSearch, FaTimes, FaPlus, FaCheck, FaChevronDown, FaChevronUp } from "react-icons/fa";
...
//   // --- NEW: Set default filters after states are loaded ---
//   useEffect(() => {
//     if (taskStates.length > 0) {
//       // Set default states (all except "Completed")
...
//       ? taskTagString.split(',').map(tag => tag.trim()).filter(Boolean)
//       : [];
//     const matchesTag = selectedTags.size === 0 || taskTagsArray.some(tag => selectedTags.has(tag));
//     return matchesState && matchesPriority && matchesUser && matchesTag;
//   });
...
//         if (statusButton) {
//           statusButton.classList.remove('ring-2', 'ring-blue-500');
//         }
//       }
...
//         if (priorityButton) {
//           priorityButton.classList.remove('ring-2', 'ring-blue-500');
//         }
//       }
...
//               <button
//                 id="status-button"
//                 className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200 bg-white hover:border-gray-400 text-left flex justify-between items-center"
//                 onClick={() => {
...
//                       className="h-4 w-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500"
//                     />
//                     <FaCheck className={`ml-2 h-4 w-4 ${selectedStates.has(state.Id.toString()) ? 'text-blue-600' : 'text-transparent'}`} />
...
//                 }}
//               >
//                 {selectedPriorities.size === 0 ? "All Priorities" : `${selectedPriorities.size} selected`}
//                 <svg className="h-4 w-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
//                   <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path>
//                 </svg>
//               </button>
...
//                       className="h-4 w-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500"
//                     />
//                     <FaCheck className={`ml-2 h-4 w-4 ${selectedPriorities.has(priority.Id.toString()) ? 'text-blue-600' : 'text-transparent'}`} />
...
//                     <span className={`ml-3 ${selectedTags.has(tag.value.toString()) ? 'font-medium text-gray-900' : 'text-gray-700'}`}>
//                       {tag.label}
//                     </span>
//                   </div>
//                 ))}
