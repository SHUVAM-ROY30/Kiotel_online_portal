...
// import { format, addDays, subDays, isToday, startOfWeek, addWeeks, subWeeks, startOfDay, endOfDay } from 'date-fns';
// // DnD Kit Imports
// import {
//   DndContext,
//   closestCenter,
...
//   // ✅ renderShiftCell with updated colors and LLOP, Festive Leave
...
//                       </thead>
//                       <tbody>
//                         {/* SortableContext Wrapper - Use filteredEmployees IDs */}
...
//               </div>
//             ) : (
//               // Display message if no schedule is loaded (e.g., no live schedule for user role)
...
//                 <button 
//                   onClick={() => {
//                     const clearData = { ...editFormData, assignment_status: 'OFF' };
//                     handleShiftEdit({ ...editTarget, ...clearData });
//                   }} 
...
//   const [currentSchedule, setCurrentSchedule] = useState(null); // Will be set automatically
//   const [scheduleEntries, setScheduleEntries] = useState([]);
//   const [myPastEntries, setMyPastEntries] = useState([]); // NEW: For month view
//   const [employees, setEmployees] = useState([]);
//   const [shiftTypes, setShiftTypes] = useState([]);
//   const [selectedWeekStart, setSelectedWeekStart] = useState(new Date());
//   const [selectedDate, setSelectedDate] = useState(new Date()); // For day view - starts on today
//   const [selectedMonth, setSelectedMonth] = useState(new Date()); // NEW: For month view - starts on current month
//   const [isDayView, setIsDayView] = useState(false); // Toggle between week/day view
//   const [isMonthView, setIsMonthView] = useState(false); // NEW: Toggle for month view
//   const [employeeOrder, setEmployeeOrder] = useState([]);
//   const [employeeSearch, setEmployeeSearch] = useState(''); // New state for search
...
//       .map(id => empMap.get(id))
//       .filter(Boolean); // Get employees in the stored order, remove invalid IDs

//     const orderSet = new Set(employeeOrder);
//     const newEmps = employees.filter(emp => !orderSet.has(emp.id)); // Find employees not in the stored order

//     // Return the ordered list: stored order first, then new employees
...
//   //       setSchedules([]);
//   //       setScheduleEntries([]);
//   //       setMyPastEntries([]); // NEW: Also clear past entries on error
//   //       setEmployeeOrder([]);
//   //     } finally {
//   //       // Set loading to false after initial fetch and potential auto-load
//   //       setLoading(false);
...
//   // }, [uniqueId, userRole]);
//   // 
//    // Removed setLoading from dependency array as it's set inside the effect
...
//         setSchedules(filteredSchedules);
//         setEmployees(empRes.data); // This state update will trigger orderedEmployees recalculation
//         setShiftTypes(shiftRes.data);

//         // Auto-load first LIVE schedule if available and user is not restricted
//         if ([1, 5].includes(userRole) || userRole === 1 || userRole === 5) {
//             const liveSchedules = allSchedules.filter(s => s.status === 'LIVE');
//             if (liveSchedules.length > 0) {
//                 const firstLiveSchedule = liveSchedules[0];
//                 setCurrentSchedule(firstLiveSchedule);
...
//                         order = firstLiveSchedule.employee_order;
//                     } else {
//                         order = empRes.data.map(e => e.id); // Use current employee list if no order set
//                     }
//                     setEmployeeOrder(order);
//                 } catch (err) {
//                     console.error('Failed to load entries for auto-loaded schedule:', err);
...
//   // ... (other functions like loadScheduleEntries, handleReorder, etc., remain the same)

//   // Filter employees based on search term - now operates on the dynamic orderedEmployees
//   const filteredEmployees = useMemo(() => {
//     if (!employeeSearch) return orderedEmployees; // Use the recalculated ordered list
//     const term = employeeSearch.toLowerCase();
//     return orderedEmployees.filter(emp => 
//       (emp.first_name.toLowerCase().includes(term) || 
//        emp.last_name.toLowerCase().includes(term))
//     );
//   }, [orderedEmployees, employeeSearch]); // Depend on orderedEmployees

//   // ... (rest of the component rendering remains the same)


//   // NEW: Load past 3 months entries when month view is enabled
//   useEffect(() => {
...
//           setMyPastEntries([]); // NEW: Clear on error
//         }
//       };
//       loadMyPastEntries();
//     }
...
//   // NEW: Generate days for the selected month
...
//     const entry = myPastEntries.find(e => e.entry_date === dateStr && Number(e.user_id) === Number(uniqueId)); // Assumes uniqueId maps to user_id
...
//       if (isMonthView) {
//           // Reload past entries for month view
//           const loadMyPastEntries = async () => {
//             try {
//               const [entriesRes] = await Promise.all([
//                 axios.get(`${API}/api/schedules/my-entries-past-3-months`, {
...
//               <div className="flex justify-end gap-4 mt-8">
//                 <button 
//                   onClick={handleClearShift} // NEW: Use the clear function
//                   className="text-rose-600 hover:text-rose-800 font-medium px-5 py-2.5 rounded-xl transition-colors"
//                 >
//                   Clear Shift
//                 </button>
