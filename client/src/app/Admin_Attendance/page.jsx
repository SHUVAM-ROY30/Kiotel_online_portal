...
function parseIST(dateStr) {
  ...
  // Convert "YYYY-MM-DD HH:mm:ss" → ISO IST
  return new Date(dateStr.replace(' ', 'T') + '+05:30');
}
function normalizeTimes(clockIn, clockOut, shiftStart, shiftEnd, isCrossMidnight) {
  let inD = parseIST(clockIn);
  let outD = clockOut ? parseIST(clockOut) : null;

  let shiftStartD = parseIST(clockIn);
  let shiftEndD = parseIST(clockIn);

  if (shiftStart) {
    const [h, m] = shiftStart.split(':');
    shiftStartD.setHours(h, m, 0, 0);
  }

  if (shiftEnd) {
    const [h, m] = shiftEnd.split(':');
    shiftEndD.setHours(h, m, 0, 0);
  }

  if (isCrossMidnight) {
    if (shiftEndD <= shiftStartD) {
      shiftEndD.setDate(shiftEndD.getDate() + 1);
    }

    // 🔥 critical fix
    if (outD && outD < shiftStartD) {
  ...
  return { inD, outD, shiftStartD, shiftEndD };
}
function calculateAttendanceDetails(
  clockIn,
  clockOut,
  shiftStart,
  shiftEnd,
  ...
  const { inD, outD, shiftStartD, shiftEndD } =
    normalizeTimes(clockIn, clockOut, shiftStart, shiftEnd, isCrossMidnight);
  ...
  // Late
  if (inD > shiftStartD) {
    const diff = Math.floor((inD - shiftStartD) / 60000);
    late = diff > grace ? diff - grace : 0;
  }

  // Early
  if (outD && outD < shiftEndD) {
    early = Math.floor((shiftEndD - outD) / 60000);
  }

  // OT
  if (outD && outD > shiftEndD) {
    ot = Math.floor((outD - shiftEndD) / 60000);
  }

  let status = 'Present';
  if (late > 0 && early > 0) status = 'Late & Early';
  else if (late > 0) status = 'Late';
  else if (early > 0) status = 'Early Clock Out';

  return {
    status,
    late_minutes: late,
    early_clock_out_minutes: early,
    overtime_minutes: ot
  };
}
